...
public class ToyFactory extends Thread{
	...
	public ToyFactory(int N, int number) {
		...
		this.factory = new boolean[N][N];			//boolean matrix that represents the factory
		this.number = number;
		this.N = N;
	}
	...
	public void run() {
		...
		while(true) {
			...
			try {
				// Tries asking elves for their position
	...
	public void moveElf(Elf elf) {
		...
		// Getting elf coordinates, status and identity
		int X = elf.getX();
		int Y = elf.getY();
	...
	private void askElvesForPosition() {
		...
		try {
			...
			// No new elves can be added while asking the current ones for their position
